    ...
    protected boolean doDecode(IoSession session, IoBuffer in,//也是mina框架中的一种字节缓冲区
        ...
        try {
            ...
            if (symbol == '+') {
                ...
                cSmessage.msgJson = new String(com.example.administrator.finalprocject.PBE.PBE.deCiphering(temp), "UTF-8");
                out.write(cSmessage);
//				Log.i("--","客户端解码："+moMsg.toString());
                ...
                cSmessage.msgJson = new String(com.example.administrator.finalprocject.PBE.PBE.deCiphering(temp1), "UTF-8");
                // 接收图片信息
                ...
                cSmessage.msgBytes = com.example.administrator.finalprocject.PBE.PBE.deCiphering(temp2);
                out.write(cSmessage);
            }
        } catch (Exception e) {
            e.printStackTrace();
