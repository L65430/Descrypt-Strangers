...
import com.example.administrator.finalprocject.PBE.PBE;
 ...
 */
public class LEncoder extends ProtocolEncoderAdapter {
    ...
    //这是用来加密的
    @Override
    public void encode(IoSession session, Object msg, ProtocolEncoderOutput out)//就是说这里的msg里面没有msgbytes
            throws Exception {
        CSmessage moMsg = (CSmessage) msg;
...
//				System.out.println("--发送文本信息   = " + moMsg.msgJson);
                ...
                byte[] msgJsonByte = PBE.enCrypt(moMsg.msgJson.getBytes());//加密
                byte[] msgBytes = PBE.enCrypt(moMsg.msgBytes);//发送第二个加密文,这里没有信息,所以无法加密//因为这里的msgbytes为空
