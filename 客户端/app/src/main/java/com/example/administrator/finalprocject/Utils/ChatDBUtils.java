 ...
 */
//用于各种在交流和调出最近列表，群组邀请等行为中的数据库操作
public class ChatDBUtils extends SQLiteOpenHelper
{
    private static final String DATABASE_NAME="L_client_db";//数据库名
...
//        System.out.print("表已经存在");//可能就是这个的问题
    ...
    {
        SQLiteDatabase db=this.getReadableDatabase();
        String sql="select * from recent_chat_table where userEmail = ? and friendId = ? ";
        Cursor cursor=db.rawQuery(sql,new String[]{String.valueOf(ClientManger.clientEmail),String.valueOf(friendid)});
        cursor.moveToNext();
    ...
    public boolean insertrecentchat(Chatinfoentry entity)
    {
        ...
        try {
            ...
            String[] args = { ClientManger.clientEmail, entity.getFriendId(),
    ...
    public void deletechatitem(String friendId)
    {
        ...
        db.execSQL(sql,new String[]{ClientManger.clientEmail,friendId});
        db.close();
    }
    ...
    public void clearrecent()
    {
        SQLiteDatabase db=this.getWritableDatabase();
        String tname="recent_chat_table";
        String sql="delete from "+tname+" where userEmail = ?";
        String paras[]={ClientManger.clientEmail};
    ...
    public List<Chatinfoentry> getrecentlist() {
        ...
        SQLiteDatabase db = this.getReadableDatabase();
        String sql = "select friendId, friendName, chatContent, chatCreatTime, msg_num, Msgtype from recent_chat_table where userEmail = ?";
        Cursor cursor = db.rawQuery(sql,
                new String[] { ClientManger.clientEmail });
        while (cursor.moveToNext()) {
    ...
    public Chatinfoentry getrecentchatfriend(String friendid)
    {
         ...
         String sql="select friendId, friendName, chatContent, chatCreatTime, msg_num, Msgtype from recent_chat_table where userEmail = ? and friendId = ?";
         Cursor cursor=database.rawQuery(sql,new String[]{ClientManger.clientEmail,friendid});
    ...
    //添加邀请
    public boolean insertinvite(InvitationInfoEntity entry)
    {
        SQLiteDatabase db=this.getWritableDatabase();
        try {
                String sql = "insert into invitations (groupId, invitorName, groupName, topic, groupIconPath) values(?, ?, ?, ? ,?)";
                db.execSQL(sql,new String[]{entry.getGroupId(),entry.getInvitorName(),entry.getGroupName(),entry.getTopic(),entry.getGroupIconPath()});
                return  true;
        }catch (Exception e)
        {
            e.printStackTrace();
