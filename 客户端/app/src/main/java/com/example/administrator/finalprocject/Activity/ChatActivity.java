...
import com.example.administrator.finalprocject.Adapter.FacePageFragmentAdapter;
import com.example.administrator.finalprocject.Adapter.MoreSelectAdapter;
...
import com.example.administrator.finalprocject.Fragment.FacePageFragment;
...
import com.example.administrator.finalprocject.Utils.SoundUtil;
import com.example.administrator.finalprocject.View.MyListView;
...
public class ChatActivity extends FragmentActivity {
   ...
   //消息数据库
    ...
    private void initData() {
        ...
        friendId = intent.getStringExtra("friendId");//这个intent是从Mainactivity传过来的
        friendName = intent.getStringExtra("friendName");
        isgroupMsg = intent.getBooleanExtra("isgroupMsg", false);
        tv_friendName.setText(friendName);

        MainActivity.mp0.setChatingId(friendId);
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
               ImageView selecView=(ImageView)view.findViewById(R.id.selec_icon);
                ...
                switch (position)
                {
                    ...
                    case 2:
                        //发送我的具体位置
                        CSmessage locMsg=new CSmessage();
                        locMsg.symbol='+';//因为传的是位置，是文本
                        JSONObject locJson=new JSONObject();
                        locJson.put(JSONKeys.msgType,CSKeys.CHATING_TEXT_MSG);
                        locJson.put(JSONKeys.userId,ClientManger.clientId);
                        locJson.put(JSONKeys.friendId,friendId);
                        locJson.put(JSONKeys.msgCotent,"我的位置："+ClientManger.myLocation);//因为这个位置可以在之后查看陌生人的时候使用
                        locJson.put(JSONKeys.sendTime,ClientUtils.getNowTime());
                        locMsg.msgJson=locJson.toJSONString();
                        if(isgroupMsg)
                        {
                           locJson.put(JSONKeys.isGroupMsg,CSKeys.GROUP_MSG);
                        }
                        ClientHandleMessage msgClient=convertionUtil.Convert_Net2Client(context,locMsg,false);//网络包变为可显示的包
                        AddMsgClientitem(msgClient);
                        MainActivity.myBinder.sendCharMsg(locMsg);
                        //保存到数据库
                        ChatMessage  msgDb=convertionUtil.Convert_Net2Db(0,1,context,locMsg);
                        MyDatabase.savechat(msgDb,friendId);
                        break;
                    case 3:

                        Toast.makeText(context,"若想了解更多信息，请登录官网：",Toast.LENGTH_LONG).show();
                        ...
                        mBaiduASRDigitalDialog.setDialogRecognitionListener(new DialogRecognitionListener() {
                           //回调函数
                            ...
                            public void onResults(Bundle results) {
                                ...
                                if(isgroupMsg)
                                {
                                    jsonObject.put(JSONKeys.isGroupMsg,CSKeys.GROUP_MSG);
                                }
                ...
                switch (event.getAction())
                {
                    ...
                    case MotionEvent.ACTION_UP:
                        ...
                        }else
                        {
                            ...
                            MainActivity.myBinder.sendCharMsg(voiceMsg);//发送(发送到mainactivity）
                            //保存到数据库
                ...
                if(isTosendTextMag)
                {
                  String content=Et_msgEdit.getText().toString();
                    if(!content.equals(""))
                    {
                        ...
                        MainActivity.myBinder.sendCharMsg(textMsg);//这是发送消息用的
                        Et_msgEdit.setText("");
                        //保存到数据库
    ...
    public void updateRecentChat() {
        ...
        if (msglist.size() > 0)//消息集合
        {
            ...
            MainActivity.mp0.updaterecentchatitem(friendId, jsonObject.getString(JSONKeys.sendTime), content, true);//数据库更改，最近联系人界面也要更新
        } else
        {
            ...
            MainActivity.mp0.updaterecentchatitem(friendId,"","",true);//就是一条消息都没有
        }
